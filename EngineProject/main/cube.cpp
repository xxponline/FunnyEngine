...
static char const *const tex_files[] = {"data/texture/lunarg.ppm"};
...
// Create a WSI surface for the window:
    ...
    vk::ShaderModule prepare_fs() {
        size_t size = 0;
        void *fragShaderCode = read_spv("data/shader/cube-frag.spv", &size);
        if (!fragShaderCode) {
    ...
    vk::ShaderModule prepare_vs() {
        size_t size = 0;
        void *vertShaderCode = read_spv("data/shader/cube-vert.spv", &size);
        if (!vertShaderCode) {
